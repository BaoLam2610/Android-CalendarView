...
class DataCalendar {
    ...
    fun getDataDates(mCalendar: Calendar,totalMonth:Int) : ArrayList<CalendarModel> {
        ...
        for (i in 0 until totalMonth){
            ...
            mapCalendar[i] = mCalendar.clone() as Calendar
            if (i == 0) {
                index = 1
            }
        }
    ...
    private fun addDataDate(mCalendar: Calendar):ArrayList<DayDataModel> {
        ...
        if ((dayNum-1)!=0){
            for (i in (icnt-(dayNum-1)) until icnt){
                ...
                val dayData = DayDataModel(year, month, i + 1, dateStr,SimpleDateFormat(
                    DatePickerKey.formatDate
                ).parse(dateStr), DatePickerKey.DAY_PREVIOUS_MONTH
                )
                data.add(dayData)
            }
        }
        ...
        if (data.size%7!=0){
            for (i in 0 until 7-(daynumNextMonth-1)){
                ...
                val dayData = DayDataModel(year, month, i + 1, dateStr,SimpleDateFormat(
                    DatePickerKey.formatDate
                ).parse(dateStr), DatePickerKey.DAY_NEXT_MONTH
                )
                data.add(dayData)
            }
        }
